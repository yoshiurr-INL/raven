...
class PPDSS(PostProcessor):
  ...
  def inputToInternal(self, currentInputs):
    ...
    if self.pivotParameterFeature in currentInputs[0].getVars('indexes'):
      ...
      for cnt in range(len(self.features)):
        ...
        for cnt2 in range(len(featureData)):
          ...
          featureOmega = np.gradient(featureBeta[cnt][cnt2],pivotFeature)
    ...
    if self.pivotParameterTarget in currentInputs[1].getVars('indexes'):
      ...
      for cnt in range(len(self.targets)):
        ...
        for cnt2 in range(len(targetData)):
          ...
          targetOmega = np.gradient(targetBeta[cnt][cnt2],pivotTarget)
    ...
    for cnt in range(len(self.features)):
      ...
      if currentInputs[0].getVarValues(self.features[0]).values.shape[1] >= currentInputs[1].getVarValues(self.targets[0]).values.shape[1]:
        y_count = currentInputs[1].getVarValues(self.targets[0]).values.shape[0]
        z_count = currentInputs[1].getVarValues(self.targets[0]).values.shape[1]
      ...
      newtargetOmegaNorm = np.zeros((len(featureProcessTimeNormScaled),len(featureProcessTimeNormScaled[0])))
      newtargetD = np.zeros((len(featureProcessTimeNormScaled),len(featureProcessTimeNormScaled[0])))
      for cnt3 in range(len(featureProcessTimeNorm[cnt])):
        ...
        if len(featureProcessTimeNormScaled[cnt3]) >= len(targetProcessTimeNorm[cnt][cnt3]):
          ...
          newfeatureOmegaNorm[cnt3] = self.interpolate(interp1dGrid,featureProcessTimeNormScaled[cnt3],featureOmegaNormScaled[cnt3])
        else:
          interp1dGrid = featureProcessTimeNormScaled[cnt3]
          newtargetOmegaNorm[cnt3] = self.interpolate(interp1dGrid,targetProcessTimeNorm[cnt][cnt3],targetOmegaNorm[cnt][cnt3])
          newtargetBeta[cnt3] = newtargetOmegaNorm[cnt3]*interp1dGrid
          newtargetD[cnt3] = self.interpolate(interp1dGrid,targetProcessTimeNorm[cnt][cnt3],targetD[cnt][cnt3])
  ...
  def interpolate(self,interp1dGrid, processTime, data):
    """
      Linearly interpolates for required data values
      @ In, interp1dGrid, 1d array grid
      @ In, processTime, 1d array grid to be interpolated
      @ In, data, 1d array data to be interpolated
    ...
    for cnt in range(len(interp1dGrid)):
      for cnt2 in range(len(processTime)):
        if cnt2 == 0:
          if interp1dGrid[cnt] < processTime[cnt2] and checkArray[cnt2] == 1:
            slope = (data[cnt2+1]-data[cnt2])/(processTime[cnt2+1]-processTime[cnt2])
            interp1dData[cnt] = data[cnt2] - slope*(processTime[cnt2]-interp1dGrid[cnt])
          elif interp1dGrid[cnt] == processTime[cnt2] and checkArray[cnt2] == 1:
            interp1dData[cnt] = data[cnt2]
          else:
            checkArray[cnt2] = 0
        elif cnt2 == len(interp1dGrid)-1:
          if processTime[cnt2] < interp1dGrid[cnt]:
            slope = (data[cnt2]-data[cnt2-1])/(processTime[cnt2]-processTime[cnt2-1])
            interp1dData[cnt] = data[cnt2] + slope*(interp1dGrid[cnt]-processTime[cnt2])
          elif interp1dGrid[cnt] == processTime[cnt2]:
            interp1dData[cnt] = data[cnt2]
          else:
            slope = (data[cnt2]-data[cnt2-1])/(processTime[cnt2]-processTime[cnt2-1])
            interp1dData[cnt] = data[cnt2] - slope*(processTime[cnt2]-interp1dGrid[cnt])
  ...
  def _writeXML(self,output,outputDictionary):
    ...
    """
    outputInstance = xmlUtils.StaticXmlElement('DssPostProcessor')
  ...
  def collectOutput(self,finishedJob, output):
    ...
    print("output asDataset():",output.asDataset())
    if output.type in ['PointSet', 'HistorySet']:
      ...
      for key, val in outputDict.items():
        ...
        rlz[newKey] = val
      if self.dynamic:
        rlz[self.pivotParameterFeature] = np.atleast_1d(self.pivotValuesFeature)
        rlz[self.pivotParameterTarget] = np.atleast_1d(self.pivotValuesTarget)
  ...
  def run(self, inputIn):
    ...
    print("Hello World! Hello World! Hello World! Hello World! Hello World! Postprocessor DSS")
    return outputDict
